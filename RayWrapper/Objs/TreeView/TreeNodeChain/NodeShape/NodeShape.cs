...
namespace RayWrapper.Objs.TreeView.TreeNodeChain.NodeShape
{
    public abstract class NodeShape
    {
        ...
        public void Update(Vector2 off, float scale, bool isSequential, bool context, bool next)
        {
            ...
            if (!isSequential)
            {
                if (isLeft) lClick?.Invoke();
                if (isRight) rClick?.Invoke();
            }
            else
            {
        ...
        public string Draw(Vector2 off, float scale)
        {
            ...
            return tooltip is null ? "" : tooltip.Invoke();
        }
    }
}