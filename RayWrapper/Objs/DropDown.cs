...
{
    public class DropDown : IGameObject
    {
        public Vector2 pos;
        ...
        public void UpdateChanges()
        {
            ...
            var back = RayWrapper.AssembleRectFromVec(pos, _size).Grow(4);
            text = new(back, options[Value]) {fontSize = fontSize};

            optionDisplay = new(new(back.x, back.y + back.height + 2),
                (int) back.width,
                i => options[i],
                () => options.Count, 4);
        ...
        public void Render()
        {
            // text.text = $"{(isListVisible ? "↑" : "↓")} {options[Value]}";
